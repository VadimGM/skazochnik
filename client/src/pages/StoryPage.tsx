...
export default function StoryPage() {
  ...
  useEffect(() => {
    ...
    const fetchStory = async () => {
      try {
        ...
        if (!res.ok) throw new Error("Сказка не найдена");
        ...
        if (data.status === "error") {
          setError("Ошибка при генерации сказки");
          setLoading(false);
          return;
        }
        setStoryData(data);
  ...
  const handleRegenerate = async () => {
    ...
    try {
      const res = await fetch(`/api/stories/${params.id}/regenerate`, { method: "POST" });
      if (!res.ok) throw new Error("Regeneration failed");

      const poll = async () => {
        const r = await fetch(`/api/stories/${params.id}`);
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-4 md:p-8 pb-20 z-10">
        ...
        {error && (
          <div className="text-center py-20">
            ...
            <a href="/" className="text-primary underline font-semibold">Создать свою сказку</a>
          </div>
        )}
